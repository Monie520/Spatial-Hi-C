...
# cis >= 1kb contact
stat=bed.loc[(bed["chrom1"]==bed["chrom2"]) & (abs(bed["mid1"]-bed["mid2"])>=1000)]["pixel"].value_counts()
raw_stat["cis_more_1kb"]=raw_stat["pixel"].apply(lambda x: stat[x] if x in stat else 0)

# cis < 1kb contact
stat=bed.loc[(bed["chrom1"]==bed["chrom2"]) & (abs(bed["mid1"]-bed["mid2"])<1000)]["pixel"].value_counts()
raw_stat["cis_less_1kb"]=raw_stat["pixel"].apply(lambda x: stat[x] if x in stat else 0)
